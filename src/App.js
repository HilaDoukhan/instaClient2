...
class App extends React.Component {
    ...
    signup = () => {
        axios.get("http://localhost:9030/signUp?username=" + this.state.username + "&password=" + this.state.password
            + "&repeat=" + this.state.repeatPassword)
            .then(response => {
                let tokenValue = "" , errorCodeValue = "";
                if (response.data.success) {
                    tokenValue = response.data.token;

                }else {

                    errorCodeValue = response.data.errorCode;
                }
                this.setState({
                    username: "",
                    password: "",
